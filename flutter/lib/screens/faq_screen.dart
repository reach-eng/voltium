...
class _FaqScreenState extends State<FaqScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredFaqs = faqs.where((f) {
      final matchesSearch = f.question.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          f.answer.toLowerCase().contains(_searchQuery.toLowerCase());
      final matchesCategory = _activeCategory == 'All' || f.category == _activeCategory;
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: isSelected ? [
                    BoxShadow(color: const Color(0xFF0053C1).withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))
                  ] : [],
                ),
                child: Text(
                  cat,
    ...
    if (filteredFaqs.isEmpty) {
      return Column(
        children: [
          ...
          const Text('No results found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
          const Text("We couldn't find any match for your search.", style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
        ],
      );
    }
  ...
  Widget _buildFaqItem(dynamic faq) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ListTile(
            onTap: () => setState(() => _expandedId = isExpanded ? null : faq.id),
            title: Text(faq.question, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
      ...
      child: Column(
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.message_outlined, color: Color(0xFF0053C1), size: 20),
              ),
              const SizedBox(width: 16),
              const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Still need help?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                  Text('Our team is available 24/7 for you.', style: TextStyle(fontSize: 11, color: Color(0xFF64748B))),
                ],
              ),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(
                child: _buildContactButton(Icons.phone_outlined, 'Call Support', const Color(0xFFECFDF5), const Color(0xFF15803D)),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _buildContactButton(Icons.email_outlined, 'Email Us', const Color(0xFFF5F3FF), const Color(0xFF7C3AED)),
              ),
            ],
          ),
        ],
  ...
  Widget _buildContactButton(IconData icon, String label, Color bg, Color text) {
    return Container(
