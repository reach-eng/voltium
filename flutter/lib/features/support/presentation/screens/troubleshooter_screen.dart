...
class _TroubleshooterScreenState extends State<TroubleshooterScreen>
  ...
  Future<void> _triggerSOS() async {
    const sosNumber = '112'; // Emergency services number
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: AnimatedSwitcher(
          ...
          child: switch (_mode) {
            _Mode.categorySelect => _buildCategorySelect(),
            _Mode.question => _buildQuestionView(),
            _Mode.result => _buildResultView(),
          },
        ),
      ),
    );
  ...
  Widget _buildQuestionView() {
    ...
    return SingleChildScrollView(
      key: const ValueKey<_Mode>(_Mode.question),
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      child: Column(
        children: [
          const SizedBox(height: 8),
  ...
  Widget _buildResultView() {
    ...
    return SingleChildScrollView(
      key: const ValueKey<_Mode>(_Mode.result),
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      child: Column(
        children: [
          const SizedBox(height: 16),
