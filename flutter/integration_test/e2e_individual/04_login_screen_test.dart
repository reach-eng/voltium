...
void main() {
  ...
  testWidgets('Login screen – enter phone and navigate to OTP', (tester) async {
    ...
    final hasLegal = find.byKey(const Key('acceptCheckbox')).evaluate().isNotEmpty;
    final hasPermissions = find.byKey(const Key('continuePermissionsButton')).evaluate().isNotEmpty;
    final hasIntent = find.byKey(const Key('deliverWithUsCard')).evaluate().isNotEmpty;
    final hasOnboarding =
    ...
    expect(
      hasOtp || hasLegal || hasPermissions || hasIntent || hasOnboarding || hasGuarantor || hasDashboard,
      isTrue,
      reason: 'Should reach OTP, Legal, Permissions, Intent, User onboarding, Guarantor onboarding, or dashboard after auth flow',
    );
  });
}
