...
void main() {
  ...
  testWidgets('Full onboarding to active dashboard — no phases skipped',
    ...
    if (app.dashboard.preDashboardScreen.evaluate().isEmpty) {
    ...
    await waitFor(tester, app.dashboard.preDashboardScreen);
    expect(
    ...
    // Tap Submit Proof
    final submitBtn = app.onboarding.submitProofButton;
    await scrollAndTap(tester, submitBtn);
    await settle(tester);

    // Wait and check if we reached the receipt screen
    final receiptScreen = app.wallet.topUpReceipt;
    ...
    await waitFor(tester, app.dashboard.choosePlan);
    expect(
      app.dashboard.choosePlan,
      findsOneWidget,
      reason: 'Phase 4b: Should be on choosePlan screen',
    );

    // Verify confirm button exists (initially disabled)
    final confirmPlan = app.dashboard.confirmPlanButton;
    ...
    final preDashboard = app.dashboard.preDashboardScreen;
    ...
    riderProvider2.updateRider(
      currentRider2!.copyWith(
        planDone: true,
    ...
    if (hubCards.evaluate().isNotEmpty) {
      ...
      final ecField = app.shared.emergencyContactField;
      ...
      // Fill OTP
      final otpFields = app.shared.otpInputField;
    ...
    rp3.updateRider(
      (rp3.rider ?? currentRider).copyWith(
        pickupDone: true,
