...
ENV_NAME="production"
# P0 fix: see deploy-staging.sh — must match ecosystem.config.js.
PM2_APP_NAME="voltium-web"
PM2_WORKER_NAME="voltium-worker"
HEALTH_ENDPOINT="${HEALTH_ENDPOINT:-http://localhost:8081/api/health}"
...
if [ "${FORCE_DEPLOY:-false}" != "true" ]; then
  if [ -t 0 ]; then
    read -r -p "⚠️  Are you sure you want to deploy to PRODUCTION? Type 'yes' to proceed: " CONFIRM
    if [ "$CONFIRM" != "yes" ]; then
      echo "Production deploy cancelled by operator."
      exit 1
    fi
  else
    echo "::error:: Deploying to production requires interactive confirmation or FORCE_DEPLOY=true." >&2
    exit 1
  fi
fi
...
# The old code logged a warning and proceeded; new code aborts the deploy.
...
# The old PM2 names (voltium-prod-web) did not exist; pm2 reload missed and fell to `pm2 start npm` (wrong cwd/model).
...
notify "❌ Deploy to $ENV_NAME FAILED: health check timeout"

if [ "$NO_ROLLBACK" = "true" ]; then
  echo "Health check failed and NO_ROLLBACK=true. Manual intervention required." >&2
  npx prisma migrate status || true
  exit 1
fi

echo "Health check failed! Rolling back CODE only (schema rolls forward — see MIGRATION_POLICY.md)..."
