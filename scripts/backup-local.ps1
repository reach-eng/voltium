...
if (-not $env:BACKUP_ENCRYPTION_KEY) {
    Write-Error "[Backup] BACKUP_ENCRYPTION_KEY env var is required for encrypted backups"
    exit 1
}

if (-not (Get-Command age -ErrorAction SilentlyContinue)) {
    Write-Error "[Backup] age CLI is required for encrypted backups"
    exit 1
}
...
# 3. Encrypt backup artifacts and delete plaintext
$encryptedArtifacts = @()
foreach ($file in @($dbFile, $uploadsFile)) {
    if (Test-Path $file) {
        $encrypted = "$file.age"
        & age -r "$env:BACKUP_ENCRYPTION_KEY" -o "$encrypted" "$file"
        if ($LASTEXITCODE -ne 0) {
            Write-Error "[Backup] Encryption failed for $file"
            exit 1
        }
...
Write-Host "[Backup] Backup artifacts encrypted" -ForegroundColor Green
...
$manifest = @{
    ...
    databaseFile = "database.sql.age"
    uploadsFile = if (Test-Path "$uploadsFile.age") { "uploads.zip.age" } else { $null }
} | ConvertTo-Json
...
$checksums = @()
if (Test-Path "$dbFile.age") {
    $hash = (Get-FileHash "$dbFile.age" -Algorithm SHA256).Hash.ToLower()
    $checksums += "$hash  database.sql.age"
}
if (Test-Path "$uploadsFile.age") {
    $hash = (Get-FileHash "$uploadsFile.age" -Algorithm SHA256).Hash.ToLower()
    $checksums += "$hash  uploads.zip.age"
}
...
# 7. Copy to secondary location (optional)
if ($SecondaryDir) {
