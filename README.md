...
> [!NOTE]
> **Codebase Structure & Package Scope:** This repository is a monorepo containing the Voltium full-stack codebase.
> - `web/`: Next.js web application, REST API endpoints, and Prisma/PostgreSQL database schema.
> - `flutter/`: Flutter mobile application for riders.
> - `scripts/`: Gate, backup, restore, and laptop service lifecycle orchestration scripts.
>
> All gate/readiness check scripts support a `--flutter-only` flag to run validation checks exclusively on the Flutter codebase, bypassing any requirements for a local Next.js web environment or database.

## Laptop Service Architecture

Voltium production is designed for a laptop-only service stack: local PostgreSQL, local uploads, local backups, PM2-managed web/worker processes, and optional Cloudflare Tunnel for routing only.
...
Full runbook: `docs/LAPTOP_SERVICE_ARCHITECTURE.md` and `docs/LAPTOP_SERVICE_RUNBOOK.md`.
...
![E2E Tests](https://img.shields.io/badge/E2E_Tests-33-green)
![Widget Tests](https://img.shields.io/badge/Widget_Tests-205_passing-brightgreen)
...
- **Rider Mobile App** (Flutter) — 59 screens covering the complete rider journey from onboarding to rental management
- **Admin Dashboard** (Next.js) — 38 screens for fleet management, KYC verification, analytics, and operations
...
- **Backend Unit Tests** — 381/381 tests passing (`npm run test:unit`)
...
# Rate limiting / jobs
# Production uses local PostgreSQL-backed rate limiting and OutboxEvent jobs.
# Do not configure Upstash/Redis/cloud queue credentials.
...
This package is prepared for a laptop-only public beta. Before inviting beta users, run:
...
Then follow `docs/PUBLIC_BETA_READINESS.md`, `docs/PUBLIC_BETA_RUNBOOK.md`, and `docs/PUBLIC_BETA_TEST_PLAN.md`.

Public beta requires local PostgreSQL, local uploads, local backups, PM2 web/worker processes, and a verified backup/restore test on the laptop.

## Screen Workflow Coverage

The admin console and rider app screen map is documented in `docs/SCREEN_WORKFLOW_COVERAGE.md`. The admin console also includes a `Workflow Coverage` section that links to every major required operating screen.

## Branding and App IDs

The public app name is **Voltium**. The old scaffold/codename has been removed from the public-beta package. See `docs/BRANDING_AND_APP_ID.md` and run `cd web && npm run check:branding` before creating builds.
