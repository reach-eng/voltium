...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const vehicle = await db.vehicle.findFirst({
      ...
      include: {
        hub: true,
        leases: { orderBy: { createdAt: 'desc' }, take: 50, include: { rider: { select: { riderId: true, fullName: true, phone: true } }, shift: true } },
        returns: { orderBy: { createdAt: 'desc' }, take: 50, include: { rider: { select: { riderId: true, fullName: true, phone: true } } } },
        incidents: { orderBy: { createdAt: 'desc' }, take: 50, include: { rider: { select: { riderId: true, fullName: true, phone: true } } } },
        tickets: { orderBy: { createdAt: 'desc' }, take: 50, include: { rider: { select: { riderId: true, fullName: true, phone: true } } } },
      },
    });
    if (!vehicle) return errors.notFound('Vehicle not found');

    return success({ vehicle, timeline: [
