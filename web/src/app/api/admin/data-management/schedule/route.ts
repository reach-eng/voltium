...
export async function GET() {
  try {
    ...
    const schedule = await dataManagementUseCases.getSchedule(session.role as AdminRole);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'test') {
      const result = await dataManagementUseCases.testSchedule(session.role as AdminRole);
      return NextResponse.json({ success: result.success, data: result });
    }

    if (action === 'run-now') {
      const result = await dataManagementUseCases.runScheduledBackupNow(session.adminId ?? '', session.role as AdminRole);
