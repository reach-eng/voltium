...
async function handleSecurityAction(rider: any, action: string, body: any, session: any): Promise<any> {
  ...
  switch (action) {
    case 'LOCK_DEVICE': fcmResult = await fcmService.sendRemoteLock(rider.fcmToken!); break;
    case 'FACTORY_RESET': fcmResult = await fcmService.sendRemoteWipe(rider.fcmToken!); break;
    ...
    case 'UNLOCK_DEVICE': {
      ...
      if (!isSuperAdmin) {
        if (!password || password !== rider.lockPassword) return errors.unauthorized('Invalid recovery password');
      }
