...
import { JOB_TO_OUTBOX_CONFIG } from '@/lib/job-outbox-config';
import { checkRateLimit, ADMIN_JOBS_RATE_LIMIT } from '@/lib/rate-limit';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit(`admin_jobs:${admin.adminId ?? 'unknown'}`, ADMIN_JOBS_RATE_LIMIT);
    if (!rateLimit.allowed) {
      return errors.tooManyRequests('Rate limit exceeded for running background jobs. Maximum 5 per hour.');
    }
