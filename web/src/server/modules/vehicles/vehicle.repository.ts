/**
 * Vehicles module - Repository.
 *
 * Data access for vehicles, fleet management, and assignment records.
 */

import { db } from '@/lib/db';

export const vehicleRepository = {
  async findAll(params?: { hubId?: string; status?: string }) {
  ...
  async update(vehicleId: string, data: Record<string, unknown>) {
  ...
  async assignToRider(vehicleId: string, riderDbId: string) {
    return db.vehicle.update({
      where: { id: vehicleId },
      data: { status: 'ASSIGNED', assignedRiderId: riderDbId },
    });
  },
  ...
  async markAvailable(vehicleId: string) {
    return db.vehicle.update({
      where: { id: vehicleId },
      data: { status: 'AVAILABLE', assignedRiderId: null },
    });
  },
  ...
  async bulkUpdateStatus(ids: string[], data: Record<string, unknown>) {
