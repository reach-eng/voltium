...
export const planUseCases = {
  async list(page: number, limit: number) {
    const [plans, total] = await Promise.all([
      db.rentalPlan.findMany({ orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: limit }),
      db.rentalPlan.count(),
    ]);
    const formatted = plans.map((p: { price: number; [key: string]: unknown }) => ({ ...p, price: paiseToRupees(p.price) }));
    return { plans: formatted, pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } };
  },

  async listActivePlans() {
    const plans = await db.rentalPlan.findMany({ where: { isActive: true }, orderBy: { price: 'asc' } });
    return plans.map((p: { price: number; [key: string]: unknown }) => ({ ...p, price: paiseToRupees(p.price) }));
  },

  async subscribeToPlan(riderDbId: string, planId: string) {
    const rider = await db.rider.findUnique({ where: { id: riderDbId }, include: { wallet: true } });
    ...
    if (!['DEPOSIT_APPROVED', 'PLAN_SELECTED', 'PICKUP_SCHEDULED', 'ACTIVE'].includes(rider.lifecycleStatus)) throw new Error('DEPOSIT_NOT_APPROVED');
    if (!rider.wallet || rider.wallet.balanceInPaise < plan.price) throw new Error('INSUFFICIENT_BALANCE');
    const now = new Date();
  ...
  async update(id: string, data: Record<string, unknown>, actorId: string) {
    ...
    createAuditLog({ actorId, action: 'plan.update', entity: 'plan', entityId: id, details: data }).catch(() => {});
    return { ...plan, price: paiseToRupees(plan.price) };
  },
