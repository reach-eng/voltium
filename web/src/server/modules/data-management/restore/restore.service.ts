...
export const restoreService = {
  async validate(backupJobId: string, adminId: string) {
    ...
    if (!job) {
      throw new NotFoundError('Backup job not found');
    }
    if (job.status !== 'COMPLETED') {
      throw new ValidationError('Cannot restore from a non-completed backup');
    }
  ...
  async startRestore(backupJobId: string, adminId: string) {
    ...
    await backupLockService.setBackupLock(true);
    ...
    try {
      ...
      if (!verification.valid) {
        throw new ValidationError(`Backup verification failed: ${verification.errors?.join(', ')}`);
      }
      ...
      if (job.databasePath && existsSync(job.databasePath)) {
        ...
        try {
          ...
          throw new ValidationError(`Database restore failed: ${dbErr.message}`);
        }
      }
      ...
      if (job.filesPath && existsSync(job.filesPath)) {
        ...
        try {
          ...
          throw new ValidationError(`Uploads restore failed: ${fileErr.message}`);
        }
      }
      ...
      await backupLockService.setBackupLock(false);
      try {
      ...
      await backupLockService.setBackupLock(false).catch(() => {});
      try {
        await setMaintenanceMode(false);
      } catch {}
