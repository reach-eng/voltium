        ...
        <div className="pt-6 border-t">
          ...
          {!hasPermission(session || {}, 'device_remote_control') ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <Card
                className={`transition-colors border-dashed ${data?.rider?.isAdminLocked ? 'bg-amber-500/10 border-amber-500' : 'bg-amber-500/5 border-amber-500/20 hover:bg-amber-500/10'}`}
              >
                <CardContent className="pt-6">
                  <div className="flex flex-col items-center text-center space-y-3">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${data?.rider?.isAdminLocked ? 'bg-amber-500 text-white animate-pulse' : 'bg-amber-500/10 text-amber-600'}`}
                    >
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <div className="flex flex-col gap-2">
                        <Button
                          ...
                          onClick={() =>
                            triggerSecurityAction('PERSIST_APP', {
                              enabled: !data?.rider?.isUninstallBlocked,
                            })
                          }
                          disabled={isActionPending}
                        >
                          {data?.rider?.isUninstallBlocked
                            ? 'Uninstall not allowed'
                            : 'Uninstall allowed'}
                        </Button>
                        <Button
                          size="sm"
                          ...
                          onClick={() =>
                            triggerSecurityAction('RESTRICT_APPS_CONTROL', {
                              enabled: !data?.rider?.isAppsControlRestricted,
                            })
                          }
                          disabled={isActionPending}
                        >
                  ...
                  <div className="flex flex-col items-center text-center space-y-3">
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          triggerSecurityAction('ENFORCE_LOCATION', {
                            enabled: !data?.rider?.isLocationMandatory,
                          })
                        }
                        disabled={isActionPending}
                      >
