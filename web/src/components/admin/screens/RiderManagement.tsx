...
export default function RiderManagement() {
  ...
  const [viewingAsRider, setViewingAsRider] = useState<string | null>(null);
  ...
  async function viewAsRider(rider: Rider) {
    setViewingAsRider(rider.id);
    const riderName = rider.fullName || 'Unknown';
    setRiderSession(rider.id, riderName);
    window.dispatchEvent(
      new CustomEvent('view-as-rider', { detail: { riderId: rider.id, riderName } })
    );
    setTimeout(() => setViewingAsRider(null), 1000);
  }
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                className="h-8 w-8 p-0 rounded-lg bg-emerald-500/5 hover:bg-emerald-500/10"
                                onClick={() => viewAsRider(rider)}
                                title="View as Rider"
                                disabled={viewingAsRider === rider.id}
                              >
                                {viewingAsRider === rider.id ? (
