 ...
 * All archive commands use native tools and use ZIP consistently:
 ...
 * Windows uses PowerShell Compress-Archive; Unix uses zip.
 * The output file must use a .zip extension.
 */
...
export function extractArchive(archiveFile: string, outputDir: string): void {
  if (IS_WIN) {
    execFileSync(
      ...
      [
        '-NoProfile',
        '-Command',
        `$ErrorActionPreference='Stop'; Expand-Archive -LiteralPath '${archiveFile.replace(/'/g, "''")}' -DestinationPath '${outputDir.replace(/'/g, "''")}' -Force`,
      ],
      { timeout: 300_000, stdio: 'pipe' }
    );
    return;
  ...
  execFileSync('unzip', ['-o', archiveFile, '-d', outputDir], {
    timeout: 300_000,
    stdio: 'pipe',
  });
}
