...
describe('Input Validation Tests', () => {
  ...
  describe('/api/rider/kyc', () => {
    it('rejects KYC submit with missing required fields', { timeout: LONG_TIMEOUT }, async () => {
      ...
      // 405 is also acceptable: the current API contract is GET-only for
      // /api/rider/kyc (KYC submission was moved to a separate endpoint).
      // Both 4xx codes prove the request was rejected. 405 responses
      // have a different body shape — only check `body.success` for
      // non-405 responses.
