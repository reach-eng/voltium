...
describe('Signed URLs — storage provider interface', () => {
  ...
  it('LocalStorageProvider returns upload URL with key param', async () => {
    const { LocalStorageProvider } = await import('@/lib/storage');
    const provider = new LocalStorageProvider();
    const url = await provider.getSignedUploadUrl('rider-123/doc.pdf', 'application/pdf');
    expect(url).toMatch(/\/api\/files\/direct-upload\?key=/);
    expect(url).toContain(encodeURIComponent('rider-123/doc.pdf'));
  });
