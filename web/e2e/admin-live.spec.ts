...
test.describe('Admin Live Payload Hooks (Phase 4)', () => {
  ...
  test('Device Tracking View — Verify GPS data renders for active riders', async ({ page }) => {
    ...
    const timeoutArm = new Promise<'TIMEOUT'>((res) => setTimeout(() => res('TIMEOUT'), 18_000));
    const authState = await Promise.race([
      dashboard.waitFor({ state: 'visible', timeout: 15_000 }).then(() => 'DASHBOARD' as const),
      loginBtn.waitFor({ state: 'visible', timeout: 15_000 }).then(() => 'LOGIN' as const),
      timeoutArm,
    ]);
    if (authState === 'TIMEOUT') throw new Error('Admin auth did not resolve within 18s');
