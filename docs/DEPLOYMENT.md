...
- `GET /api/health` — Basic health check (DB, PostgreSQL OutboxEvent, disk)
...
- local logging is configured via `LOCAL_LOGGING_ONLY` env var
- Errors are logged locally even without local logging
...
- [ ] local logging DSN configured (optional)
