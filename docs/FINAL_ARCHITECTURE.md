...
Request
  │
  ├──► Rate Limiting (PostgreSQL OutboxEvent + in-memory fallback)
  │
...
| Error tracking   | local logging (lazy-loaded via LOCAL_LOGGING_ONLY) |
| APM              | In-memory metrics (request timing, slow queries) |
| Health checks    | `GET /api/health` — DB, PostgreSQL OutboxEvent, disk, uptime |
