...
- **T-AR-SORT · Step 3** (P2, F-2) — `history_screen.dart` (Flutter) replaced its own string-matching `'All'/'Credits'/'Debits'` filter + search with the shared `TransactionFilterSort` widget + `TransactionFilter` enum; added `history_searchHint` l10n. Eliminated "Credit" vs "Credits" label drift. Server `type` param wiring is a TODO (needs backend + OpenAPI regen).
- **T-AR-SORT · Step 4** (P2, F-3) — `SortDropdown<T>` upgraded to enum-typed `PopupMenuButton` with `tooltip` + `sortActive` tint. Existing callers passing `List<String>` declare `<String>` explicitly.
- **T-AR-SORT · Step 5** (P2, A-3) — `GET /api/admin/transactions` now reads `?sortBy` (allowlist: `createdAt` / `amount`) and `?sortDir` (`asc` / `desc`), threads through to the use-case; `cacheKey` includes them; repo translates `amount` → `amountInPaise` (the persisted column). `useTransactions.ts` sends the params, cycles 3-state (none → desc → asc → none, matching the data-table primitive), resets to page 1 on sort change, and dropped the misleading client-side `sorted` block. `TransactionTable.tsx` Amount + Date headers now expose `aria-sort` and lucide `ArrowUp` / `ArrowDown` / `ArrowUpDown` (no more literal `↑` / `↓` glyphs — same encoding-loss class as A-1).
- **Pre-existing bug fixed along the way** — `flutter/lib/features/notifications/presentation/screens/notifications_screen.dart` referenced `AppLocalizationsEn()` without importing `gen/app_localizations_en.dart`. Dart 3 can't synthesize the concrete class from the abstract base; the import was added.
- **New tests** — `tests/unit/data-table-aria-sort.test.tsx` (4), `tests/unit/rider-table-aria-sort.test.tsx` (6), `tests/unit/transaction-table-aria-sort.test.tsx` (8), `flutter/test/wallet/history_filter_sort_test.dart` (11), `flutter/test/wallet/sort_dropdown_active_tint_test.dart` (4).
...
**Shipped 2026-08-24 (Flutter rider pages audit — top 10 findings, 10 tickets T-110..T-119):**

Audit doc: `docs/AUDIT_FLUTTER_PAGES_2026-08-24.md` (47 screens catalogued, 28 active AuthStates, 48 files with hardcoded English, 1 dead-loop battery drain).

- **T-110 (P0) Notifications tabs are English-only** — `notifications_screen.dart:118-131` hardcodes `'All'/'Payments'/'KYC'/'Maintenance'/'Announcements'`. The KYC/Maintenance tabs ALSO filter by `.toLowerCase().contains('kyc'/'verification'/'document'/'service'/'maintenance')` on notification titles — Hindi notifications never appear in those tabs. Fix: localise tab labels + change filter to use a dedicated `category` field on the notification (server-side addition) instead of title-keyword matching.
- **T-111 (P0) 8 AppBar titles hardcoded English** — `wallet_screen.dart:91` ('Wallet'), `support_center_screen.dart:56` ('Support Center'), `notifications_screen.dart:399` ('Notifications'), FAQ category chips ('All'), `emergency_sos_screen.dart` (title), `kyc_preflight_screen.dart` (headings), `login_screen.dart` tagline (after 'Voltium' proper noun), dashboard greeting `txtgreetingMorning/Afternoon/Evening`. Fix: replace literals with `l10n.txt*` keys (keys already exist for most — just need to be used at the call site).
- **T-112 (P0) No Logout / Sign out button on Profile menu** — Settings has only "Delete Account" (GDPR-style). A rider who lends their phone cannot switch accounts. Fix: add a non-destructive "Sign out" entry to the Profile menu between Settings and Delete; show a confirmation dialog; calls the same `logout()` the session-expired path uses.
- **T-113 (P0) Support phone/email fallbacks are hardcoded fake values** — `support_center_screen.dart:43-44` falls back to `'+919876543210'` and `'support@voltium.app'`; same fallback duplicated in `faq_screen.dart:30-37`. If `supportConfig` is null (e.g. fresh install before admin publishes config), tapping "Call Support" dials a fake number. Fix: hide the Call/Email buttons when `supportConfig == null` + show a "Support is being configured. Please try again shortly." message.
- **T-114 (P0) Error messages in critical flows are hardcoded English** — `user_onboarding_screen.dart:953` ('Session expired. Please log in again.'), `:962` ('Server temporarily unavailable. Please try again later.'); `create_ticket_screen.dart:320, 373` ('Please enter a subject'/'Please enter a message'). Fix: replace with `l10n.txt*` keys; add `txtsessionExpired`, `txtserverUnavailable`, `txtsubjectRequired`, `txtmessageRequired` to ARB. (LANGUAGE-AUDIT already unblocked 475 keys — these are the residual critical paths.)
- **T-115 (P0) `pickup_hub_screen.dart` is 919 lines / 9 fields / no retry / no batch upload** — the largest screen in the app, mixes image-picker + emergency-contact-OTP + draft-persistence + state validation. Each of the 5 photo tiles issues its own file-upload request serially (a slow network = 5 sequential timeouts). Vehicle dropdown list is cached across hub changes (a stale selection from a previous hub can leak). Fix: extract `_PhotoTiler` widget that uploads in parallel; add per-photo retry icon; invalidate vehicle cache on hub change.
- **T-116 (P0) `hang_tight_screen.dart` polls every 15s forever** — battery drain for riders waiting 4+ hours for admin approval. ~960 polls. No backoff, no offline detection, no max-iteration. Fix: exponential backoff (15s/30s/60s/120s/300s/600s), pause when `ConnectivityProvider.isOffline == true`, subscribe to the lifecycle gate's event so the poll stops the moment the rider is activated.
- **T-117 (P1) 48 files contain hardcoded English strings** — bulk i18n pass. Linter rule needed: `'[A-Z][a-z]{2,}[ ][a-z]+'` in `feature/**/presentation/screens/` triggers a warning. Most are non-critical (error toasts, helper text), but the i18n drift is large. Fix: add a custom `flutter_lints` rule that flags any string literal starting with a capital letter in a presentation file outside `*.arb`, batch-add ARB keys + callsite updates via `flutter gen-l10n`.
- **T-118 (P1) Permissions screen has no "Skip" / "I'll do this later"** — a rider who accidentally denies Notifications (or any required permission) is stuck. Per audit comment, "every permission is now compulsory". Fix: add a "Skip for now" button that records the consent as denied in `ConsentService` (already tracks all consents) and routes to the next screen; the rider can come back via the Settings "Re-grant permissions" affordance.
- **T-119 (P1) TopUp flow is 3 screens for 1 action** — `top_up_amount_screen.dart` + `top_up_proof_screen.dart` + `top_up_receipt_screen.dart` is over-engineered for a low-frequency "add money" task. Fix: collapse to 1 screen with a stepper (amount input → photo upload → submit button). Receipt becomes a SnackBar / inline confirmation card. `top_up_flow.dart` already wraps them; the wrapper can become a single StatefulScreen with internal step state.

**Out of scope for the page audit:** widget folders (`features/*/widgets/`), auth lifecycle gate, individual dashboard card widgets. Will be audited if/when a real rider-visible issue is reported.

**Shipped 2026-08-24 (Flutter rider pages audit — PR-1..PR-5):**

Audit doc: `docs/AUDIT_FLUTTER_PAGES_2026-08-24.md` (47 screens catalogued, 10 tickets T-110..T-119).

- ✅ **PR-1 (T-110 + T-111)** — localise the 5 notification tab labels (`txtnotifTabAll/Payments/Kyc/Maintenance/Announcements`, en+hi) and replace the title-keyword filter with a `TODO(server)` note. Localise the Wallet, Support Center, Notifications, FAQ AppBar titles and the "All" chip on the wallet filter and FAQ category. Add `txtfaq` ARB key. New test: existing `faq_screen_test.dart` updated for the new "FAQ" title.
- ✅ **PR-2 (T-112)** — add a non-destructive "Sign out" entry to the Profile menu (between Emergency SOS and the bottom padding). Confirmation dialog with a "data is safe" copy. Calls the same `RiderNotifier.logout()` path the session-expired flow uses. New ARB keys: `menu_signOut`, `menu_signOutConfirmTitle/Body/Action`, `menu_signOutCancelled`. New test: `test/profile_signout_test.dart` (Cancel does not log out, Confirm invokes logout once).
- ✅ **PR-3 (T-113)** — drop the hardcoded `+919876543210` / `support@voltium.app` fallbacks in `support_center_screen.dart` and `faq_screen.dart`. The Call/Email contact cards now hide when the corresponding `SupportConfig` field is empty. New "Support is being configured" info card renders in the Support Center when both fields are empty. New ARB key: `supportNotConfigured`. New test: `test/support/support_config_visibility_test.dart` (4 Support Center scenarios + 1 FAQ scenario).
- ✅ **PR-4 (T-114)** — localise the KYC submit error messages in `user_onboarding_screen.dart` (401, 403, 408, 504, 5xx, network). Localise the create-ticket field labels, hint text, and validator messages in `create_ticket_screen.dart` (Subject, Message, "Please enter a subject", "at least 5 characters", "at least 10 characters"). New ARB keys (en+hi): `txtkycErrorInvalidDocs`, `txtkycErrorAccessDenied`, `txtkycErrorUploadTimeout`, `txtserverUnavailable`, `txtgenericTryAgain`, `txtsubject/Required/TooShort`, `txtmessage/Required/TooShort`, `txtdescribeIssueHint`. New test: `test/kyc_error_format_test.dart` (7 cases: every status code path + ARB contract).
- ✅ **PR-5 (T-116)** — `hang_tight_screen.dart` now uses exponential-backoff polling (15s → 30s → 60s → 120s → 240s → 300s cap) instead of a flat 15s cadence. Skips ticks when the device is offline (`connectivityProvider.isOnline == false`). Resets backoff on a successful refresh AND on a manual pull-to-refresh (added `RefreshIndicator` to the scroll view). New test: `test/hang_tight_backoff_test.dart` (4 cases: schedule walk, cap, 4h energy savings ≈ 30 polls vs 960).

**Final Flutter test count: 1617 passing, 7 skipped, 0 failing** (was 1581 + 7 + 0 pre-audit; +28 new tests from PR-1..PR-5, +8 from l10n gen sweep).

**Shipped 2026-08-24 (Flutter rider pages audit — PR-6 + PR-7):**

Continuing the audit-ticket ship plan from PR-1..PR-5 above.

- ✅ **PR-6 (T-118)** — `permissions_screen.dart` now shows a "Skip for now" TextButton below the Continue button when at least one required permission is denied. Tapping it surfaces a confirmation dialog (Cancel / "Skip anyway"). On confirm, the same `onNext` callback the Continue button uses is invoked — the router doesn't know whether the rider skipped or fully granted. Re-grant affordance lives in Settings. New ARB keys: `txtskipForNow`, `txtskipPermissionsConfirmTitle/Body/Action`. New test: `test/permissions_skip_test.dart` (3 cases: dialog opens, Cancel does not navigate, Confirm invokes onNext once).
- ✅ **PR-7 (T-119)** — collapsed the TopUp flow from 3 screens (amount → proof → receipt) to 2. The router's `topUpProof.onSubmit` now navigates directly to `AuthState.dashboard` (or `AuthState.pickupHub` during onboarding) instead of `AuthState.topUpReceipt`. The success snackbar the proof screen already shows is sufficient feedback — the receipt screen's "Back to Dashboard" button was a tap the rider shouldn't have to make. The `topUpReceipt` state is kept in the enum (other code still references it) but the production router no longer routes through it. New test: `test/topup_receipt_skip_test.dart` (1 case: pins the contract that topUpReceipt is no longer the post-submit target).

**Final Flutter test count: 1621 passing, 7 skipped, 0 failing** (was 1617 post-PR-5; +4 from PR-6/PR-7).

**Remaining audit tickets (deferred to a follow-up session — both are large, multi-file refactors that need dedicated time):**
- **PR-8 (T-115)** — `pickup_hub_screen.dart` is 919 lines, the largest screen in the app. Hold for a dedicated refactor session that splits it into `_PhotoTiler` (parallel photo uploads), `VehicleCacheManager` (per-hub cache invalidation), and a state machine. Estimated 2-3 hours of focused work.
- **PR-9 (T-117)** — bulk i18n pass: 48 files still contain hardcoded English strings. Needs a custom `flutter_lints` rule + batch ARB key generation. Estimated 3-4 hours; better done as a standalone audit pass with grep-driven ARB key generation.
